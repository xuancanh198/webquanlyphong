...
export const getListFunService = (objectGet) => {
  return (dispatch) => {
    ...
    const linkGroup = objectGet?.linkGroup && objectGet?.linkGroup === 'user' ? "user" : "admin"
    const apiUrl = `${linkGroup}/${objectGet.routerLink}?page=${objectGet.page}&limit=${objectGet.limit}${searchParam}${fitterParam}${excelParam}${filtersBase64Param}${filterBaseDecodeParam}`;
    APILink.get(apiUrl)
      .then((response) => {
